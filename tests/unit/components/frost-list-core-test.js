...
describeComponent(
  ...
  function () {
    ...
    describe('"Ember.assert" in "Init" hook', function () {
      const selection = {
        onSelect: function () {}
      }
      const expansion = {
      ...
      beforeEach(function () {
        EmberAssertSpy = sinon.spy(Ember, 'assert')
      })

      afterEach(function () {
        Ember.assert.restore()
      })

      it('is NOT called', function () {
        run(() => { component.trigger('init') })

        expect(
          EmberAssertSpy.called,
          'Ember.assert is NOT called'
        ).to.be.false
      })

      it('is called once when "sorting" presented', function () {
        run(() => {
          component.set('sorting', sorting)
        ...
        expect(
          EmberAssertSpy.called,
          'Ember.assert is called with error message'
        ).to.be.true

        expect(
          EmberAssertSpy.callCount,
          'Ember.assert is called once'
        ).to.eql(1)
      })

      it('is called twice when "sorting" and "expansion" presented', function () {
        run(() => {
          component.set('sorting', sorting)
          component.set('expansion', expansion)
          component.trigger('init')
        ...
        expect(
          EmberAssertSpy.callCount,
          'Ember.assert is called twice'
        ).to.eql(2)
      })

      it('is called 3 times when "sorting", "expansion" and "selection" presented', function () {
        run(() => {
          component.set('sorting', sorting)
          component.set('expansion', expansion)
        ...
        expect(
          EmberAssertSpy.callCount,
          'Ember.assert is called 3 times'
        ).to.eql(3)
      })
    })
  }
